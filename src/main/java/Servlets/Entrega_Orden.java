 ...
 */
public class Entrega_Orden extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(neg != null){
                vcli = aux.Ordenes_Actuales(neg.getId_mneg());
                pagina = "error_neg.jsp";
                valor = request.getParameter("Orden");
                if(vcli != null){
                    longitud = vcli.size();
                    if(val.Validar_Delegacion(valor, longitud)){
                        VWOrden clie = vcli.elementAt(Integer.parseInt(valor));
                        estatus = MOrden.Cambio_Entregado(clie.getId_mord());
                        if(estatus > 0){
                            pagina = "ordenes.jsp"; 
                        }
                    }
