...
$(document).ready(function(){
    $("form").submit(function(e){
        ...
        var cost = document.getElementById('Monto').value
        if(!regex_plat.test(nom) || nom.length > 50 || nom.length < 4){
            alert("El titulo del pago solo debe de tener caracteres alfanumericos y debe de ser de 4 a 50 caracteres")
            e.preventDefault()
            return false
        }     
        if(!regex_plat.test(desc) || desc.length > 200 || desc.length < 4){
            alert("La descripcion del pago solo debe de tener caracteres alfanumericos y debe de ser de 4 a 200 caracteres")
            e.preventDefault()
            return false
        }
